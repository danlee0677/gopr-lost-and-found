...
void PostScreen() {
    // error msg
    if (postError) draw_text(postErrorMsg[postError], WIDTH - 50, 30, 40, TOP_RIGHT, RED);
    
    // title
    draw_rectangle(50, 85, WIDTH - 100, 55, TOP_LEFT, (postSelected == 1 ? RED : BLACK));
    draw_text(TextFormat("%s", postTitle), 57, 112.5, 40, MIDDLE_LEFT, BLACK);
    ...
    draw_rectangle(50, 220, WIDTH - 100, HEIGHT - 360 - 220, TOP_LEFT, (postSelected == 2 ? RED : BLACK));
    ...
    draw_rectangle(50, HEIGHT - 280, WIDTH - 100, 50, TOP_LEFT, (postSelected == 3 ? RED : BLACK));
    draw_text(postTargetUser, 55, HEIGHT - 255, 40, MIDDLE_LEFT, BLACK);
    ...
    for (int i = 0; i < 6; i++) {
        draw_rectangle(50 + (30 + tagWidth) * i, HEIGHT - 160, tagWidth, 50, TOP_LEFT, (postTagsSelected[i] ? RED : BLACK));
        draw_text(tagNames[i], 50 + (tagWidth + 30) * i + tagWidth / 2, HEIGHT - 135, 30, MIDDLE_CENTER, BLACK);
    }
