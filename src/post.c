...
void post_screen() {
    ...
    // title
    draw_rectangle(50, 85, WIDTH - 100, 55, TOP_LEFT, (g_post_selected == 1 ? RED : BLACK));
    draw_text(TextFormat("%s", g_post_title), 57, 112.5, 40, MIDDLE_LEFT, BLACK);
    ...
    draw_rectangle(50, 220, WIDTH - 100, HEIGHT - 360 - 220, TOP_LEFT, (g_post_selected == 2 ? RED : BLACK));
    ...
    for (int i = 0; i < 6; i++) {
        draw_rectangle(50 + (30 + tag_width) * i, HEIGHT - 160, tag_width, 50, TOP_LEFT, (g_post_tags_selected[i] ? RED : BLACK));
...
void post_reset() {
    ...
    memset(g_post_target_user, '\0', MAX_USERNAME_LEN);
    memset(g_post_tags_selected, false, 6);
    g_post_error = 0;
    g_post_selected = 0;
}
...
bool post_valid() {
    if (strlen(g_post_title) == 0) {
        g_post_error = 1;
        return false;
    } else if (strlen(g_post_content) == 0) {
        g_post_error = 2;
        return false;
    } else if (strlen(g_post_target_user) != 0 && strlen(g_post_target_user) != 5) {
        g_post_error = 3;
        return false;
    }
    g_post_error = true;
